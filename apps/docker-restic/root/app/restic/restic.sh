...
function resticbackup() {
   ...
   $(which restic) backup --quiet --tag "${RESTIC_TAG}" --repo "${RESTIC_REPOSITORY}" --password-command "$(which echo) ${RESTIC_PASSWORD}" --host "${RESTIC_HOST}" --cache-dir "${RESTIC_CACHE_DIR}" --cleanup-cache --pack-size "${RESTIC_PACK_SIZE}" --exclude-file "${RESTIC_EXCLUDES}" --option rclone.args="serve restic --stdio --addr :80 --checkers=16 --tpslimit 10 --drive-chunk-size=32M --dropbox-chunk-size=32M --drive-use-trash=false --fast-list --config=/config/rclone/rclone.conf" "${RESTIC_FOLDER}" &>/dev/null
   if [[ "$?" == "0" ]]; then
      if [[ "${NOTIFICATION_LEVEL}" == "ALL" ]]; then
